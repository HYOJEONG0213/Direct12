...
void RenderTargetGroup::OMSetRenderTargets(uint32 count, uint32 offset)
{
	...
	D3D12_RECT	   rect = D3D12_RECT{0, 0, static_cast<LONG>(_rtVec[0].target->GetWidth()),
...
void RenderTargetGroup::WaitTargetToResource() { GRAPHICS_CMD_LIST->ResourceBarrier(_rtCount, _targetToResource); }

void RenderTargetGroup::WaitResourceToTarget() { GRAPHICS_CMD_LIST->ResourceBarrier(_rtCount, _resourceToTarget); }