...
struct BoneWeight
{
	...
	void AddWeights(uint32 index, double weight)
	{
		...
		auto findIt =
			std::find_if(boneWeights.begin(), boneWeights.end(), [=](const Pair &p) { return p.second < weight; });
		...
		// 가중치는 최대 4개
		if (boneWeights.size() > 4) boneWeights.pop_back();
	}
...
struct FbxMeshInfo
{
	...
	vector<vector<uint32>>	indices; // 다중 인덱스 버퍼
...
private:
	...
	void LoadOffsetMatrix(FbxCluster *cluster, const FbxAMatrix &matNodeTransform, int32 boneIdx,
						  FbxMeshInfo *meshInfo);
	void LoadKeyframe(int32 animIndex, FbxNode *node, FbxCluster *cluster, const FbxAMatrix &matNodeTransform,
					  int32 boneIdx, FbxMeshInfo *container);
