...
void FBXLoader::ParseNode(FbxNode *node)
{
	...
	for (int32 i = 0; i < childCount; ++i) ParseNode(node->GetChild(i));
}
...
void FBXLoader::LoadMaterial(FbxSurfaceMaterial *surfaceMaterial)
{
	...
	// 색상 추출
	material.diffuse =
		GetMaterialData(surfaceMaterial, FbxSurfaceMaterial::sDiffuse, FbxSurfaceMaterial::sDiffuseFactor);
	material.ambient =
		GetMaterialData(surfaceMaterial, FbxSurfaceMaterial::sAmbient, FbxSurfaceMaterial::sAmbientFactor);
	material.specular =
		GetMaterialData(surfaceMaterial, FbxSurfaceMaterial::sSpecular, FbxSurfaceMaterial::sSpecularFactor);
...
{
	if (mesh->GetElementNormalCount() == 0) return;
...
Vec4 FBXLoader::GetMaterialData(FbxSurfaceMaterial *surface, const char *materialName, const char *factorName)
{
	...
	Vec4 ret = Vec4(static_cast<float>(material.mData[0] * factor), static_cast<float>(material.mData[1] * factor),
					static_cast<float>(material.mData[2] * factor), static_cast<float>(factor));
...
wstring FBXLoader::GetTextureRelativeName(FbxSurfaceMaterial *surface, const char *materialProperty)
{
	...
	if (textureProperty.IsValid())
	{
		...
		if (1 <= count)
		{
			...
			if (texture) name = texture->GetRelativeFileName();
		}
	}
	...
	for (size_t i = 0; i < _meshes.size(); i++)
	{
		for (size_t j = 0; j < _meshes[i].materials.size(); j++)
		{
...

			{
				...
				if (diffuseTexture) material->SetTexture(0, diffuseTexture);
			}

			{
				...
				if (normalTexture) material->SetTexture(1, normalTexture);
			}

			{
				...
				if (specularTexture) material->SetTexture(2, specularTexture);
			}
...
void FBXLoader::LoadBones(FbxNode *node, int32 idx, int32 parentIdx)
{
	...
	const int32 childCount = node->GetChildCount();
	for (int32 i = 0; i < childCount; i++) LoadBones(node->GetChild(i), static_cast<int32>(_bones.size()), idx);
}
...
void FBXLoader::LoadAnimationData(FbxMesh *mesh, FbxMeshInfo *meshInfo)
{
	...
	if (skinCount <= 0 || _animClips.empty()) return;
...
void FBXLoader::LoadOffsetMatrix(FbxCluster *cluster, const FbxAMatrix &matNodeTransform, int32 boneIdx,
...
void FBXLoader::LoadKeyframe(int32 animIndex, FbxNode *node, FbxCluster *cluster, const FbxAMatrix &matNodeTransform,
							 int32 boneIdx, FbxMeshInfo *meshInfo)
{
	if (_animClips.empty()) return;
...
int32 FBXLoader::FindBoneIndex(string name)
{
	...
	{
		if (_bones[i]->boneName == boneName) return i;
	}
